      ...
      try {
        const response = await fetch(
          "http://127.0.0.1:8000/get_incentives_data",
          {
            ...
            body: JSON.stringify({
              query: `Give me list of all incentives and grants schemes granted by government applicable for ${product} and in this ${region} and write #Cost: followed by the final cost after applying for these schemes that I'm passing in Array with product value ${productValue}`,
            }),
          },
        );
    ...
    <div className="w-full h-full overflow-y-auto p-5">
      ...
      <div className="flex flex-col gap-5 mt-5">
        {incentiveList.map((item) => (
          <div className="border border-black rounded p-2 w-[500px]">
            {item}
          </div>
        ))}
      </div>
      ...
      <Input
        ...
        placeholder={
          curValue > 0
            ? "Calculating cost..."
            : "Enter and click submit to start calculating.."
        }
      />
      <div className="mt-10 flex flex-col gap-5 w-28">
        <Button onClick={() => setProductValue(curValue)}>
          Calculate Cost
        </Button>
        <Button>Next</Button>
      </div>
